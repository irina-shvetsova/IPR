...
def page_upload() -> None:
    ...
    st.write(
        "Загрузи CSV-выгрузку (колонки: тип, название, оценка, комментарий) "
        "или xlsx-выгрузку платформы («Оценки» + «Комментарии»). "
        "Числа принимаются и с точкой, и с запятой."
    )

    uploaded = st.file_uploader("Файл с результатами 360°", type=["csv", "xlsx"])
