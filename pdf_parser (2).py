...
СОСТАВ И ПОРЯДОК КОЛОНОК (С/ВСЕ/Л/КК) СВОИ У КАЖДОГО ОТЧЁТА: колонка группы
оценивающих появляется, только если в ней 2+ человек (иначе, по правилам
отчёта, она не показывается отдельно, но входит в общую оценку окружения).
Поэтому число и порядок колонок не жёстко заданы — на каждой странице читаем
готовую строку-легенду вида "С ВСЕ Л КК" (она есть перед каждой таблицей) и
ищем в ней позицию "ВСЕ" — см. _find_column_order.
...
# Короткая строка вида 'С ВСЕ Л КК' или 'ВСЕ Л КК' — набор и порядок колонок
# свои для каждого отчёта (зависят от того, кто из групп прошёл порог в 2+
# оценивших, см. "Об отчёте"). Не полагаемся на фиксированный список
# аббревиатур — распознаём по форме: строка из коротких (≤4 буквы) слов
# ЗАГЛАВНЫМИ буквами. Полная легенда с расшифровкой ("С Самооценка ВСЕ Все
# окружение...") не подходит для этой цели — там есть слова длиннее 4 букв.
_COLUMN_HEADER_LINE = re.compile(r"^[А-ЯЁ]{1,4}(?:\s+[А-ЯЁ]{1,4}){0,5}$")


def _find_column_order(page_text: str) -> list[str] | None:
    """Возвращает колонки страницы в их реальном порядке, например ['С','ВСЕ','Л','КК']."""
    for line in page_text.split("\n"):
        line = line.strip()
        if _COLUMN_HEADER_LINE.match(line):
            tokens = line.split()
            if "ВСЕ" in tokens:
                return tokens
    return None
...
def _split_row(raw_row_text: str, all_index: int, n_cols: int) -> tuple[str, float] | None:
    """
    ...
    all_index/n_cols — позиция колонки "Все окружение" и общее число колонок
    на ЭТОЙ странице, определённые через _find_column_order: набор и порядок
    колонок (С/ВСЕ/Л/КК и в каком составе) свой у каждого отчёта и даже может
    отличаться между категориями внутри одного отчёта.
    ...
    или где число баллов не совпадает с ожидаемым — отбрасываются.
    """
    ...
    scores: list[str] = []
    i = len(tokens)
    while i > 0 and len(scores) < n_cols and _SCORE_TOKEN.match(tokens[i - 1]):
        scores.insert(0, tokens[i - 1])
        i -= 1
    if len(scores) != n_cols:  # меньше ожидаемого — не можем надёжно сопоставить колонки
        return None
    ...
    all_score_raw = scores[all_index].rstrip("!?")
    if all_score_raw == "НД":
        return None
    try:
    ...
    with pdfplumber.open(BytesIO(file_bytes)) as pdf:
        ...
        for page in pdf.pages:
            ...
            if category in (None, "ignore"):
                continue
            columns = _find_column_order(text)
            if columns is None:
                continue  # не нашли легенду колонок — не с чем сопоставлять баллы
            all_index = columns.index("ВСЕ")
            n_cols = len(columns)
            for table in page.extract_tables():
                for row in table:
                    result = _split_row(" ".join(c for c in row if c), all_index, n_cols)
                    if result is None:
                        continue
                    name, score = result
