...
def page_generate() -> None:
    ...
    with m3:
        with st.container(border=True):
            ...
            st.markdown(f"#### {len(profile.competencies)}")
    ...
    if st.button("Сформировать ИПР", type="primary"):
        ...
        stage_titles = {
            "контекст": "Контекст развития и зоны роста",
            "направления": "Направления развития",
            "обучение и контроль": "Обучение и вопросы для саморефлексии",
        }
        status = st.status("Генерация плана…", expanded=True)
        progress_bar = status.progress(0.0)
        stage_line = status.empty()

        def on_progress(done: int, total: int, chunk_name: str) -> None:
            """Показывает текущий этап генерации и продвигает полосу."""
            progress_bar.progress(done / total)
            if done < total:
                title = stage_titles.get(chunk_name, chunk_name)
                stage_line.write(f"Этап {done + 1} из {total}: {title}")
            else:
                stage_line.write("Собираем документ…")

        generator = build_generator(api_key, folder_id, model_id)

        # Пошаговый прогресс работает, если generate() принимает колбэк progress.
        # Если версия генератора старее — показываем общий статус без обрыва.
        import inspect
        if "progress" in inspect.signature(generator.generate).parameters:
            result = generator.generate(profile, intent, progress=on_progress)
        else:
            stage_line.write("Идёт генерация, обычно занимает около минуты…")
