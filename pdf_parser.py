...
Для СВОБОДНОГО ТЕКСТА комментариев так лечить нельзя: пробовал схлопывать
подряд идущие короткие фрагменты — ломает короткие настоящие слова («ты», «не»,
«на»), которых в русском языке много и они неотличимы от обрывков без словаря.
Поэтому комментарии оставлены как есть, с редкими лишними пробелами внутри слов.
Для той роли, которую они играют (иллюстрация для LLM, а не текст для читателя
"как есть"), это не критично — смысл считывается и с артефактом.
"""
