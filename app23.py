...
from xlsx_parser import parse_360_report
from pdf_parser import parse_360_pdf
...
st.markdown(
    ...
    <style>
        ...
        [data-testid="block-container"] {{ padding-top: 2.2rem; }}
...
def page_upload() -> None:
    ...
    st.write(
        "Загрузи свой PDF-отчёт 360° (тот, что прислали на почту), CSV-выгрузку "
        "(колонки: тип, название, оценка, комментарий) или xlsx-выгрузку платформы "
        "(«Оценки» + «Комментарии»). Числа принимаются и с точкой, и с запятой."
    )

    uploaded = st.file_uploader("Файл с результатами 360°", type=["pdf", "csv", "xlsx"])
    if uploaded is not None:
        name_lower = uploaded.name.lower()
        try:
            if name_lower.endswith(".pdf"):
                fio, profile = parse_360_pdf(uploaded.getvalue())
            elif name_lower.endswith(".xlsx"):
                profiles = parse_360_report(uploaded.getvalue())
                if not profiles:
                    st.warning("В файле не нашлось участников на листе «Оценки».")
                    return
                if len(profiles) == 1:
                    fio, profile = next(iter(profiles.items()))
                else:
                    fio = st.selectbox(
                        "В файле несколько участников — выбери, для кого собираем план",
                ...
                fio = ""
        except Exception as exc:  # noqa: BLE001 — файл грузит любой сотрудник,
            # а не только вы; битый/неожиданный файл не должен ронять приложение
            # необработанным traceback'ом. Раньше здесь ловился только ValueError —
            # достаточно было, пока файлы разбирали вы сами и знали их формат.
        ...
        st.session_state["profile_fio"] = fio
        usage_log.log_upload(
...
def _show_profile(profile: Profile360) -> None:
    ...
    total_competencies = len(profile.base_competencies) + len(profile.managerial_competencies)
    col1.metric("Компетенции", total_competencies)
    ...
    with st.container(border=True):
        ...
        st.caption(
            "В анализ идут базовые и менеджерские компетенции, оценки ролей и деструкторы "
            "(деструкторы — как часть зон риска наравне с низкими компетенциями, не отдельным списком)."
        )
        if profile.base_competencies:
            st.markdown("Базовые компетенции")
            st.markdown(_items_to_html(profile.base_competencies), unsafe_allow_html=True)
        if profile.managerial_competencies:
            st.markdown("Менеджерские компетенции")
            st.markdown(_items_to_html(profile.managerial_competencies), unsafe_allow_html=True)
        if profile.roles:
            st.markdown("Роли")
            st.markdown(_items_to_html(profile.roles), unsafe_allow_html=True)

        risk = profile.risk_zones()
        if risk:
            st.markdown("Зоны риска (низкие компетенции + выраженные деструкторы)")
            rows = "".join(
                f"<tr><td>{name}</td><td class='num'>{f'{score:.1f}'.replace('.', ',')}</td></tr>"
                for name, score in risk
            )
            st.markdown(
                f"<table class='ipr-table'><thead><tr><th>Показатель</th>"
                f"<th class='num'>Оценка</th></tr></thead><tbody>{rows}</tbody></table>",
                unsafe_allow_html=True,
            )

        if profile.strength_comments:
            st.markdown("Комментарии коллег — сильные стороны")
            for c in profile.strength_comments:
                st.markdown(f"- **[{c.author_role}]** {c.text}")

        if profile.growth_comments:
            st.markdown("Комментарии коллег — зоны развития")
            for c in profile.growth_comments:
                st.markdown(f"- **[{c.author_role}]** {c.text}")

        if profile.general_comments:
            st.markdown("Комментарии коллег (без деления на сильные стороны/развитие)")
            for c in profile.general_comments:
                prefix = f"**[{c.author_role}]** " if c.author_role else ""
                st.markdown(f"- {prefix}{c.text}")

        comment_count = (
            len(profile.strength_comments) + len(profile.growth_comments) + len(profile.general_comments)
        )
        if comment_count:
            st.caption("Комментарии используются только как иллюстрация к цифрам, не как источник отдельных выводов.")
...
def page_choices() -> None:
    ...
    with st.container(border=True):
        ...
        full_name = c1.text_input(
            "ФИО сотрудника",
            value=answers.get("full_name", "") or st.session_state.get("profile_fio", ""),
    ...
    with nav_next:
        if st.button("Далее → Генерация", type="primary"):
            ...
            usage_log.log_choices(st.session_state["session_id"], st.session_state["intent_raw"])
            goto_step(3)
...
def page_generate() -> None:
    ...
    with m1:
        with st.container(border=True):
            st.caption("Направление")
            st.markdown(f"#### {raw['direction'].capitalize()}")
    with m2:
        with st.container(border=True):
            st.caption("Готовность")
            st.markdown(f"#### {raw.get('readiness', 'Средняя')}")
    with m3:
        with st.container(border=True):
            st.caption("Компетенций")
            total_competencies = len(profile.base_competencies) + len(profile.managerial_competencies)
            st.markdown(f"#### {total_competencies}")

    st.info(
        "Генерация занимает до 10 минут — это нормально. Не закрывай вкладку, "
        "пока идёт сборка плана.",
        icon=None,
    )
    if st.button("Сформировать ИПР", type="primary"):
        ...
        # Генерация не принимает и не вызывает никаких колбэков в интерфейс:
        # запросы к модели идут параллельно в фоновых потоках, а Streamlit
        # обновляется только из главного потока. Поэтому статус — статичный,
        # без пошагового прогресса.
        status = st.status("Генерация плана… обычно занимает 3–4 минуты", expanded=True)
...
def _show_result(data: dict, raw: dict) -> None:
    ...
    if s7.get("questions"):
        ...
        for q in s7["questions"]:
            st.markdown(f"- **{q.get('quarter','')}:** {q.get('question','')}")
